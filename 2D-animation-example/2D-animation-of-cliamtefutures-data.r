...
tas <- tidync("/rdsi/private/climatefutures/ACCESS1-0/50km/tas_3hr_ccam_wine_access1-0_50km.2000-2009.nc") %>%
  hyper_filter(time = index < 366) %>%
  hyper_tibble()
clh <- tidync("/rdsi/private/climatefutures/ACCESS1-0/50km/clh_3hr_ccam_wine_access1-0_50km.2000-2009.nc") %>%
  hyper_filter(time = index < 366) %>%
  hyper_tibble()
clm <- tidync("/rdsi/private/climatefutures/ACCESS1-0/50km/clm_3hr_ccam_wine_access1-0_50km.2000-2009.nc") %>%
  hyper_filter(time = index < 366) %>%
  hyper_tibble()
cll <- tidync("/rdsi/private/climatefutures/ACCESS1-0/50km/cll_3hr_ccam_wine_access1-0_50km.2000-2009.nc") %>%
  hyper_filter(time = index < 366) %>%
  hyper_tibble()
...
plotted_layers <- ggplot(data=tas, aes(lon, lat, frame=time, fill=tas)) + 
  ...
  geom_raster(data=cll, aes(lon, lat, alpha=cll/300, fill=NULL), fill="grey90", interpolate = TRUE) + 
  geom_raster(data=clm, aes(lon, lat, alpha=clm/300, fill=NULL), fill="grey95", interpolate = TRUE) +
  geom_raster(data=clh, aes(alpha = clh/300), fill="white", interpolate = TRUE) +
  scale_alpha_continuous(guide=FALSE, limits=c(0,1))

gganimate(
  plotted_layers, 
  "/mnt/WineAustralia_working_files/animations/ANIMATION-50km-1hrly-surface-temperature-with-cloud-at-lmh.gif", 
  interval = 0.23
)
